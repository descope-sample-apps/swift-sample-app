...
func totpSignUp (userEmail: String, loginId: String, userPhone: String, name: String, completionHandler: @escaping (Bool, TOTPResponse?, DescopeError?) -> Void) async throws {
    ...
    Task {
        do {
            let totpResponse = try await Descope.totp.signUp(loginId: loginId.lowercased(), details: details)
...
func enchantedSignUp (userEmail: String, loginId: String, userPhone: String, name: String, completionHandler: @escaping (Bool, EnchantedLinkResponse?, DescopeError?) -> Void) async throws {
    ...
    Task {
        do {
            enchantedResponse = try await Descope.enchantedLink.signUp(loginId: loginId, details: details, uri: "https://example.com")
...
func enchantedLinkUpdateEmail (loginId: String, email: String, completionHandler: @escaping (Bool, EnchantedLinkResponse?, DescopeError?) -> Void) async throws {
    Task {
        do {
            enchantedResponse = try await Descope.enchantedLink.updateEmail(email, loginId: loginId, uri: "https://example.com", refreshJwt: descopeSession!.refreshJwt, options: UpdateOptions(rawValue: 1))
...
func enchantedLinkSignUpOrIn (loginId: String, completionHandler: @escaping (Bool, EnchantedLinkResponse?, DescopeError?) -> Void) async throws {
    ...
    Task {
        do {
            enchantedResponse = try await Descope.enchantedLink.signUpOrIn(loginId: loginId, uri: "https://example.com")
...
func enchantedLinkSignIn (loginId: String, completionHandler: @escaping (Bool, EnchantedLinkResponse?, DescopeError?) -> Void) async throws {
    ...
    Task {
        do {
            enchantedResponse = try await Descope.enchantedLink.signIn(loginId: loginId, uri: "https://example.com")
...
func magicLinkSignUp (userEmail: String, loginId: String, userPhone: String, name: String, method: String, completionHandler: @escaping (Bool, DescopeError?) -> Void) async throws {
    ...
    Task {
        do {
            ...
            print("Login ID: " + loginId)
            if method == "email" {
                try await Descope.magicLink.signUp(with: .email, loginId: loginId, details: details, uri: "https://example.com")
            } else if method == "sms" {
                try await Descope.magicLink.signUp(with: .sms, loginId: loginId, details: details, uri: "https://example.com")
            }
...
func magicLinkSignUpOrIn (loginId: String, method: String, completionHandler: @escaping (Bool, DescopeError?) -> Void) async throws {
    ...
    Task {
        do {
            if method == "email" {
                try await Descope.magicLink.signUpOrIn(with: .email, loginId: loginId, uri: "https://example.com")
            } else if method == "sms" {
                try await Descope.magicLink.signUpOrIn(with: .sms, loginId: loginId, uri: "https://example.com")
            }
            print("Successfully initiated Magic Link SignUp")
            completionHandler(true, nil)
        } catch let descopeErr as DescopeError {
...
func magicLinkSignIn (loginId: String, method: String, completionHandler: @escaping (Bool, DescopeError?) -> Void) async throws {
    ...
    Task {
        do {
            if method == "email" {
                try await Descope.magicLink.signIn(with: .email, loginId: loginId, uri: "https://example.com")
            } else if method == "sms" {
                try await Descope.magicLink.signIn(with: .sms, loginId: loginId, uri: "https://example.com")
            }
...
func magicLinkUpdateEmail (loginId: String, email: String, completionHandler: @escaping (Bool, DescopeError?) -> Void) async throws {
    Task {
        do {
            try await Descope.magicLink.updateEmail(email, loginId: loginId, uri: "https://example.com", refreshJwt: descopeSession!.refreshJwt, options: UpdateOptions(rawValue: 1))
...
func magicLinkUpdatePhone (loginId: String, phone: String, method: String, completionHandler: @escaping (Bool, DescopeError?) -> Void) async throws {
    Task {
        do {
            if method == "sms" {
                try await Descope.magicLink.updatePhone(phone, with: .sms, loginId: loginId, uri: "https://example.com", refreshJwt: descopeSession!.refreshJwt, options: UpdateOptions(rawValue: 1))
                print("Successfully started Magic Link Email Update")
                completionHandler(true, nil)
            }
            // Later add whatsapp
...
func passwordsSignUp (userPassword: String, userEmail: String, loginId: String, userPhone: String, name: String, method: String, completionHandler: @escaping (Bool, DescopeError?) -> Void) async throws {
    ...
    Task {
        do {
            print(details)
            print("Login ID: " + loginId)
            try await Descope.password.signUp(loginId: loginId, password: userPassword, details: details)
